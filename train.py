...
def gradient_descent(X_train, Y_train, X_val, Y_val, W, b, eta):
	...
	for e in range(epochs):
		...
		for i in range(0, X_train.shape[0], mini_batch_size):
			...
			#L2-regularisation
			for i in range(len(g_W_mini)):
				g_W_mini[i] = g_W_mini[i] + lambda_reg*W[i]
			for i in range(len(g_b_mini)):
				g_b_mini[i] = g_b_mini[i] + lambda_reg*b[i]	
...
def train_network(X, Y, X_val, Y_val, N, k, eta, activation):
	...
	W = [np.random.uniform(-0.5, 0.5, (p[1], p[0])) for p in zip(Nshift, newN)]
	b = [np.random.uniform(-0.5, 0.5, (1, p)) for p in newN]
...
# 	img_aug1 = vertical_flip(img)
# 	img_aug2 = random_rotation(img)
# 	img_aug3 = random_rotation(img)
# 	img_aug4 = cutout(img, img.shape[0] // 2)
# 	img_aug5 = cutout(img, img.shape[0] // 2)
# 	img_aug = np.reshape(img_aug, 784)
# 	img_aug1 = np.reshape(img_aug1, 784)
# 	img_aug2 = np.reshape(img_aug2, 784)
# 	img_aug3 = np.reshape(img_aug3, 784)
# 	img_aug4 = np.reshape(img_aug4, 784)
# 	img_aug5 = np.reshape(img_aug5, 784)
# 	X_train_aug.append(img_aug)
# 	X_train_aug.append(img_aug1)
# 	X_train_aug.append(img_aug2)
# 	X_train_aug.append(img_aug3)
# 	X_train_aug.append(img_aug4)
# 	X_train_aug.append(img_aug5)
# 	for j in range(6):
...
# labels_train = np.concatenate((labels_train,labels_train_aug))
#Convert to one-hot
...
W, b = train_network(X_train, Y_train, X_val, Y_val, N, k, eta, activation)
...
(a_val, h_val, fx_val) = forward_prop(X_val, W, b)
y_cap = (np.argmax(fx_val, axis = 1))

#print([a for a in zip(labels_val, y_cap)])
...
if(test_file != None):
	...
	for x_test in X_test:
		(a_test, h_test, fx_test) = forward_prop(x_test, W, b)
		y_cap.append(np.argmax(fx_test))

	#print([a for a in y_cap))
